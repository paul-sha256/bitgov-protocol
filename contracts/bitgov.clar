...
;; Casts weighted vote on active governance proposal
...
;; Emergency pause mechanism for contract operations
...
;; Returns current total STX locked in protocol
(define-read-only (get-stx-pool)
    (ok (var-get stx-pool))
)

;; Returns total number of governance proposals created
